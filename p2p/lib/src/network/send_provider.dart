...
import 'package:p2p/src/model/state/send/sending_file.dart';
import 'package:p2p/src/network/http_service.dart';
...
final deviceFullInfoProvider = Provider<Device>((ref) {
  throw 'Device info not initialized. Override deviceFullInfoProvider in RefenaScope.';
});
...
class SendNotifier extends Notifier<Map<String, SendSessionState>> {
  ...
  Future<void> startSession({
      ...
      try {
        ...
        switch (e.statusCode) {
          case 401:
            ...
            // TODO: Handle PIN dialog purely via state or callback, currently just failing for refactor
            pin = null;
            // Logic to request PIN from UI should be injected or handled via state change listening
    ...
    if (target.version == '1.0') {
      ...
      if (response.statusCode == 204) {
        ...
        try {
          final responseDto =
              PrepareUploadResponseDto.fromJson(response.bodyToJson);
  ...
  Future<void> _sendLoop(Ref ref, String sessionId, Device target,
  ...
  void _finish({required String sessionId}) {
    ...
    if (state[sessionId]!.status != SessionStatus.sending) {
      ...
      final hasError = sessionState.files.values
  ...
  Future<bool> sendFile({
            ...
            files: s.files.map((key, value) {
              if (key == file.file.id) {
                return MapEntry(
                    key,
                    value.copyWith(
                        status: FileStatus.queue, errorMessage: null));
              }
        ...
        state: (s) {
          ...
          return s.copyWith(
            sendingTasks: s.sendingTasks
                ?.where((task) => !(task.isolateIndex == isolateIndex &&
    ...
    if (isRetry) {
      final sessionState = this.state[sessionId];
      if (sessionState != null &&
          sessionState.files.values.map((e) => e.status).isFinishedOrError) {
        _finish(sessionId: sessionId);
        return false;
      }
    }
  ...
  void closeSession(String sessionId) {
    ...
    if (sessionState.status == SessionStatus.finished &&
        ref.read(p2pSettingsProvider).sendMode == SendMode.single) {
