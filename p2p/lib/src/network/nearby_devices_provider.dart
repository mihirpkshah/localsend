...
import 'package:p2p/src/model/state/nearby_devices_state.dart';
import 'package:p2p/src/network/favorites_provider.dart';
...
final nearbyDevicesProvider =
  ...
  return NearbyDevicesService(
    ...
    discoveryLogs: ref.read(discoveryLoggerProvider),
  );
});
...
class RegisterDeviceAction
  ...
  Future<NearbyDevicesState> reduce() async {
    ...
    if (favoriteDevice != null && !favoriteDevice.customAlias) {
      ...
      external(notifier._favoriteService).dispatch(
...
class RegisterSignalingDeviceAction
  ...
  NearbyDevicesState reduce() {
    ...
    final existingDevice = existingDevices
        .firstWhereOrNull((e) => e.signalingId == device.signalingId);
    ...
    return state.copyWith(
      signalingDevices: {
        for (final entry in state.signalingDevices.entries)
          entry.key:
              entry.value.where((e) => e.signalingId != signalingId).toSet(),
      },
    );
  }
}
